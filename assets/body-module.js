...
function bodyTextFormat(template, params = {}) {
  return String(template || '').replace(/\{(\w+)\}/g, (_, key) => String(params[key] ?? ''));
}

function t(lang, key, params = {}) {
  const locale = ['zh', 'en', 'es'].includes(lang) ? lang : currentLang();
  const template = BODY_TEXTS[locale]?.[key] || BODY_TEXTS.zh[key] || key;
  return bodyTextFormat(template, params);
}
...
function drawSceneBackdrop(ctx, width, height) {
  ...
  wash.addColorStop(0, 'rgba(245, 249, 255, 0.98)');
  wash.addColorStop(0.55, 'rgba(235, 243, 255, 0.82)');
  wash.addColorStop(1, 'rgba(224, 234, 252, 0.76)');
  ctx.fillStyle = wash;
  ctx.fillRect(0, 0, width, height);

  const halo = ctx.createRadialGradient(width * 0.52, height * 0.24, 8, width * 0.52, height * 0.24, width * 0.42);
  halo.addColorStop(0, 'rgba(134, 184, 255, 0.28)');
  halo.addColorStop(0.42, 'rgba(152, 136, 255, 0.12)');
  halo.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle = halo;
  ctx.fillRect(0, 0, width, height);

  const sideGlow = ctx.createRadialGradient(width * 0.18, height * 0.2, 10, width * 0.18, height * 0.2, width * 0.34);
  sideGlow.addColorStop(0, 'rgba(255,255,255,0.76)');
  sideGlow.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle = sideGlow;
  ctx.fillRect(0, 0, width, height);

  drawStageGuideRings(ctx, width, height);
...
function drawStageGuideRings(ctx, width, height) {
  ctx.save();
  ctx.strokeStyle = 'rgba(114, 145, 210, 0.14)';
  ctx.lineWidth = 1;
  ctx.setLineDash([7, 10]);
  [0.16, 0.24, 0.32].forEach((scale, index) => {
    ctx.beginPath();
    ctx.ellipse(width * 0.52, height * (0.84 + index * 0.025), width * scale, height * (0.022 + index * 0.008), 0, 0, Math.PI * 2);
    ctx.stroke();
  });
  ctx.restore();
}
...
function drawCurrentFigure(ctx, p, snapshot, width, height) {
  ...
  fill.addColorStop(0, 'rgba(213, 227, 255, 0.94)');
  fill.addColorStop(0.42, 'rgba(255, 255, 255, 0.99)');
  fill.addColorStop(1, 'rgba(194, 214, 255, 0.92)');
  drawCoreGlow(ctx, p);
  ctx.save();
  ctx.shadowColor = 'rgba(94, 128, 204, 0.22)';
  ctx.shadowBlur = 32;
  ...
  ctx.restore();
  drawFigureDetailLines(ctx, p);
  drawHighlights(ctx, p, snapshot);
}

function drawCoreGlow(ctx, p) {
  ctx.save();
  const glow = ctx.createRadialGradient(p.centerX, p.topY + p.bodyHeight * 0.38, 16, p.centerX, p.topY + p.bodyHeight * 0.42, p.hipHalf * 2.15);
  glow.addColorStop(0, 'rgba(255,255,255,0.72)');
  glow.addColorStop(0.38, 'rgba(168, 204, 255, 0.22)');
  glow.addColorStop(1, 'rgba(168, 204, 255, 0)');
  ctx.fillStyle = glow;
  ctx.fillRect(p.centerX - p.hipHalf * 2.4, p.topY - 40, p.hipHalf * 4.8, p.bodyHeight + 120);
  ctx.restore();
}

function drawFigureDetailLines(ctx, p) {
  ctx.save();
  ctx.strokeStyle = 'rgba(126, 154, 214, 0.20)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(p.centerX, p.topY + p.headR * 2.15);
  ctx.lineTo(p.centerX, p.topY + p.bodyHeight * 0.98);
  ctx.stroke();
  [
    { y: p.topY + p.bodyHeight * 0.24, half: p.chestHalf * 0.82 },
    { y: p.topY + p.bodyHeight * 0.42, half: p.waistHalf * 0.78 },
    { y: p.topY + p.bodyHeight * 0.56, half: p.hipHalf * 0.84 },
  ].forEach((band) => {
    ctx.beginPath();
    ctx.moveTo(p.centerX - band.half, band.y);
    ctx.lineTo(p.centerX + band.half, band.y);
    ctx.stroke();
  });
  ctx.restore();
}
...
/* ===== v25 core-only body patch 20260421 ===== */
Object.assign(BODY_TEXTS.zh, {
  hubMusic: '数据中心',
  hubBody: '围度记录',
  hubProfile: '身体状态',
  ...
  hubOpen: '点人体展开核心功能',
  hubClose: '收起入口',
  heroClickHint: '拖动查看 3D 人体，点一下展开身体、饮食、拍照和数据入口。',
});
Object.assign(BODY_TEXTS.en, {
  hubMusic: 'Data hub',
  hubBody: 'Measurements',
  hubProfile: 'Body status',
  hubFood: 'Food bank',
  hubCapture: 'Photo analysis',
  hubToday: 'Today',
  hubOpen: 'Tap body for core tools',
  hubClose: 'Hide tools',
  heroClickHint: 'Drag around the 3D body. Tap once to open body, intake, photo, and data tools.',
});
Object.assign(BODY_TEXTS.es, {
  hubMusic: 'Datos',
  hubBody: 'Medidas',
  hubProfile: 'Estado corporal',
  hubFood: 'Biblioteca',
  hubCapture: 'Análisis por foto',
  hubToday: 'Hoy',
  hubOpen: 'Toca el cuerpo',
  hubClose: 'Ocultar',
  heroClickHint: 'Arrastra para ver el cuerpo 3D. Tócalo para abrir cuerpo, ingesta, foto y datos.',
});
handleBodyHubActionV20 = function handleBodyHubActionV25Core(action) {
  setBodyHubOpenV20(false);
  if (action === 'body') {
    window.__haochijiaOpenBodyPanel?.(true);
    return;
...
const legacySaveBodyRecordCoreOnly = saveBodyRecord;
saveBodyRecord = function saveBodyRecordCoreOnly() {
  legacySaveBodyRecordCoreOnly();
  setBodyPanelState(true, false);
  refreshBodyActionHubCopyV20();
};
window.addEventListener('haochijia:body-ready', () => {
  refreshBodyActionHubCopyV20();

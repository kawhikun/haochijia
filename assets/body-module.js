...
function drawSceneBackdrop(ctx, width, height) {
  ...
  wash.addColorStop(0, 'rgba(244, 249, 255, 0.99)');
  wash.addColorStop(0.55, 'rgba(232, 242, 255, 0.9)');
  wash.addColorStop(1, 'rgba(220, 232, 255, 0.82)');
  ctx.fillStyle = wash;
  ctx.fillRect(0, 0, width, height);

  const halo = ctx.createRadialGradient(width * 0.52, height * 0.24, 8, width * 0.52, height * 0.24, width * 0.42);
  halo.addColorStop(0, 'rgba(134, 184, 255, 0.36)');
  halo.addColorStop(0.42, 'rgba(152, 136, 255, 0.18)');
...
function drawStageGuideRings(ctx, width, height) {
  ctx.save();
  ctx.strokeStyle = 'rgba(114, 145, 210, 0.2)';
  ctx.lineWidth = 1;
...
function figureProfile(record, angle, width, height) {
  ...
  const bodyHeight = Math.min(height * 0.88, Math.max(height * 0.8, (record.heightCm / 175) * height * 0.82));
...
function drawCurrentFigure(ctx, p, snapshot, width, height) {
  ...
  fill.addColorStop(0, 'rgba(205, 224, 255, 0.98)');
  fill.addColorStop(0.42, 'rgba(255, 255, 255, 0.99)');
  fill.addColorStop(1, 'rgba(184, 210, 255, 0.96)');
  drawCoreGlow(ctx, p);
  ctx.save();
  ctx.shadowColor = 'rgba(94, 128, 204, 0.28)';
  ctx.shadowBlur = 42;
...
function drawCoreGlow(ctx, p) {
  ...
  glow.addColorStop(0.38, 'rgba(168, 204, 255, 0.3)');
...
function createWebGLBodyScene(canvas, THREE, OrbitControls) {
  ...
  const hemi = new THREE.HemisphereLight(0xf2f6ff, 0x253246, 1.72);
  scene.add(hemi);

  const key = new THREE.DirectionalLight(0xffffff, 2.45);
  key.position.set(3.6, 5.2, 4.2);
  key.castShadow = true;
  ...
  const rim = new THREE.DirectionalLight(0x8fa8ff, 1.42);
  rim.position.set(-4.0, 3.1, -4.1);
  scene.add(rim);

  const fill = new THREE.PointLight(0xe8f2ff, 1.36, 12, 2);
  fill.position.set(0, 1.7, 2.4);
  scene.add(fill);

  const floor = new THREE.Mesh(
    new THREE.CircleGeometry(2.28, 72),
    new THREE.ShadowMaterial({ transparent: true, opacity: 0.24, color: 0x7f9be0 })
  );
  ...
  const glowRing = new THREE.Mesh(
    new THREE.RingGeometry(1.14, 2.02, 72),
    new THREE.MeshBasicMaterial({ color: 0x8aa7ff, transparent: true, opacity: 0.2, side: THREE.DoubleSide })
  );
  ...
  function animate(time) {
    ...
    controls.update();
    glowRing.material.opacity = 0.16 + Math.sin(time / 950) * 0.03;
    focusHalo.material.opacity = bodyState.focusField ? 0.15 + Math.sin(time / 210) * 0.07 : 0.02;
    if (currentGroup) {
      currentGroup.position.y = Math.sin(time / 980) * 0.015;
